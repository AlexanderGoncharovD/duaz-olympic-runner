...
public class LocationGeneration : MonoBehaviour
{
    ...
    public GameObject prefabEmptySprite; // пустая балванка для спрайта
    ...
    /*Генерация объектов декорации в пределах одного чанка*/
    void ChunkGeneration ()
    {
        ...
        for (int i = 0; i < NumberOfChanks; i++)
        {
            ...
            if (Random.Range(0.0f, 1.0f) < valueGenerateBush)
            {
                ...
                newDecoration.name = "groundRock_" + i;
                newDecoration.transform.parent = ParentDecoration;
            }
            Chunk.position = new Vector3(Chunk.position.x + SizeChunk.x, Chunk.position.y, Chunk.position.z);
        }
    }
    ...
    GameObject CreateDecoration (Sprite sprite)
    {
        ...
        newObject.GetComponent<SpriteRenderer>().sortingOrder = -81 - layer;

        // Удаление использованной ячейки из списка оставшихся доступных ячеек
        PossibleCells = "";
        for (int i = 0; i < AllCells.Length; i++)
        {
            if (i == rndIndexCell || AllCells[i] == "" || AllCells[i].Substring(0, AllCells[1].Length - 1) == cell.x+ ""/*исключает стобец*/) 
            {
            }
            else
            {
