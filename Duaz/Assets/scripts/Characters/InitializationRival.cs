...
public class InitializationRival : MonoBehaviour {
    ...
    private float extraWite = 1.0f; // Дополнительное ожидание сауна соперников

    // Инициализация соперников для забега
    public IEnumerator InstantiateRival()
    {
       ...
       CreatingAnOpponentOnTheSecondTreadmill(2, "Line2", false);

        if (isInternetConnection)
        {
            isDoneRivels = new bool[3];
            yield return new WaitForSeconds(Random.Range(0.5f, 1.0f));
            CreatingAnOpponentOnTheSecondTreadmill(1, "Line1", true);
            yield return new WaitForSeconds(Random.Range(0.0f, 1.0f));
            CreatingAnOpponentOnTheSecondTreadmill(4, "Line4", true);
        }
        else
        {
    ...
    private void CreatingAnOpponentOnTheSecondTreadmill(int idTreadmill, string sortingLayerName, bool isDelay)
    {
